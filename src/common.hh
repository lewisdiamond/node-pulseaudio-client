...
#define EXCEPTION(_type_, _isolate_, _msg_) Exception::_type_(String::NewFromOneByte(_isolate_, (const uint8_t*)_msg_).ToLocalChecked())
...
#define DefineConstant(_isolate_, target, name, constant)                          \
  (target)->DefineOwnProperty(_isolate_->GetCurrentContext(),                      \
                String::NewFromOneByte(_isolate_, (const uint8_t*) #name).ToLocalChecked(),         \
